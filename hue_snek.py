...
"practice-project" inspired by Phue (Nathanaël Lécaudé)
...
class Light(object):
    ...
    def __repr__(self):
        """__repr__.
        Changes the way an instance is represented
        """
    ...
    def get(self, *args):
        """get.
        Pass through function for getting light info
        Args:
            args: (str type) e.g. "name", "on", "bri", etc.
            See Hue Api for all options
        """
    ...
    def set(self, *args):
        """set.
        Pass trough function to set light properties
        Args:
            args: (str type) e.g. "on", "bri", "hue"
            See Hue Api for all options
        """
...
class Hue(object):
    ...
    def checkup(self):
        """checkup.
        Checks connection to the bridge by making
        a http request and then seeing if the API throws any error
        """
        # basic connection and username check
        try:
    ...
    def req(self, mode='GET', address=None, data=None):
        ...
        if address is None:
            address = self.address  # use standard address (see __init__)
        try:
            # get request
            if mode == 'GET':
                ret = json.loads(
    ...
    def get_light(self, light_id=None, param=None):
        ...
        Args:
            ...
            param: The Parameter that should be returned (e.g. "name", "on").
            Leave unset if everything should be returned
        """
        # get light info
        if light_id is None:
            return 1
        ...
        # check if parameter is in main body
        # or in state sub-body (see json structure)
    ...
    def get_lights(self, mode='obj'):
        ...
        Args:
            mode: The information the function returns
            (str type: obj, id, name, model-id)
        """

        # gets all lights based on selected mode (standard is object mode)
        # and returns dict
    ...
    def get_group(self, group_id=None, param=None):
        ...
        Args:
            ...
            param: The parameter that should be returned
            (leave unset if everything should be returned)
        """
    ...
    def get_groups(self, mode='all'):
        ...
        # gets all groups based on selected mode (standard is all mode)
        # and returns dict
    ...
    def get_scenes(self, mode='all'):
        ...
        # gets all scenes based on selected mode (standard is all mode)
        # and returns dict
    ...
    def set_scene(self, group_id=1, scene_name=None):
        """set_scene.
        Sets a scene for a specified group (by id)
    ...
    def get_bridge_info(self):
        """get_bridge_info.
        Returns bridge's name, id, mac-address,
        model-id and api-version as dict
        """
